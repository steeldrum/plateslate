...
/*
plateslate/
getServerDataAsXMLbyProfile.php
tjs 120221
...
*/
// for test:
//http://localhost/~thomassoucy/plateslate/getServerDataAsXMLbyProfile.php?account=1&profile=elderly
...
$clientMember = Member::getMember($clientUserAccount);
$profileAccount = $account;
$profileFirstName = "unknown";
$profileLastName = "unknown";
$profile = $_GET['profile'];
if ($profile == "aahfInfo") {
	//$member = Member::getMember($clientUserAccount);
	  $profileFirstName = $clientMember->getValueEncoded( "firstName" );
	  $profileLastName = $clientMember->getValueEncoded( "lastName" );
} else {
	list( $members, $totalRows ) = Member::getByPrimarySkillArea( $profile );
	$rowCount = 0;
	// TODO randomly select a member or TBD use selction criteria that favor sponsor members
	foreach ( $members as $member ) {
	  $rowCount++;
	  $profileAccount = $member->getValueEncoded( "id" );
	  $profileFirstName = $member->getValueEncoded( "firstName" );
	  $profileLastName = $member->getValueEncoded( "lastName" );
	}
}
// use the profile user's account if one was found else the client user's account is used
$account = $profileAccount;
$isProfile = $profileAccount != $clientUserAccount;

// update this client's profile
if ($isProfile) {
	$clientMember->updatePrimarySkillArea($profile);
}
...
echo "<tables accountId=\"".$clientUserAccount."\" isProfile=\"".$isProfile."\" profileFirstName=\"".$profileFirstName."\" profileLastName=\"".$profileLastName."\">"; 
